...
MiddleboxFW::MiddleboxFW()
{
}

int
MiddleboxFW::configure(Vector<String> &conf, ErrorHandler* errh) {
	// Parsing
	if (Args(conf, errh)
...
int MiddleboxFW::initialize(ErrorHandler *errh)
{
	click_chatter("===============================================\n");
	click_chatter("Batch size\t:\t%d\n", batch_element_size);
	click_chatter("Test use pkt\t:\t%d\n", maxPktUsed);
	...
	PatternLoader::load_firewall_file(firewallFilePath,rules);
	click_chatter("Load %d filewall rules.\n", rules.size());

	boxTotalTime = 0;
	return 0;
}

void MiddleboxFW::push(int port, Packet * p_in)
{
	...
	if (!VeriTools::isTestPacket(p_in))
	{
		if (justSend)
		{
			...
			VeriTools::reDirectionPacket(p, boxFW_src_ip, boxFW_src_mac, boxFW_dst_ip, boxFW_dst_mac);
	...
	{
		//########################  different in each box
		VeriTools::initBoxBatch(batch, pveri->batchID, batchBasedPkt ? pktBasedVerify : flowBasedVerify, FW);
	...
	else
	{
		...
		// filter ip and port
		reader.attach(p);
		in_addr srcIP = reader.getSrcIP();
		in_addr dstIP = reader.getDstIP();
		uint32_t numSrcIP = ntohl(*(uint32_t*)&srcIP);
		uint32_t numDstIP = ntohl(*(uint32_t*)&dstIP);
		for (auto iter = rules.begin(); iter != rules.end(); iter++)
		{
			VeriTools::checkRule(numSrcIP, numDstIP, *iter);
		}
		VeriTools::reDirectionPacket(p, boxFW_src_ip, boxFW_src_mac, boxFW_dst_ip, boxFW_dst_mac);
		ready_packet.push_back(p);
	...
	if (batch.readyToSendRoot)
		if (batch.batchPktSize == batch.packetCount)
		{
			...
			else
			{
				...
				{
					VeriTools::fflowFW(batch.flows[(uint32_t)iterRes->first], iterRes->second);
				}
				buildTreeAndSendRootPkt(batch);
			}
		}
...
Packet* MiddleboxFW::pull(int port) {
	Packet* p = 0;
...
void MiddleboxFW::buildTreeAndSendRootPkt(boxBatch & batch)
{
	...
	//########################  different in each 
	VeriTools::reDirectionPacket(pktRoot, boxFW_src_ip, boxFW_src_mac, boxFW_dst_ip, boxFW_dst_mac);
	if (verbose)
...
CLICK_ENDDECLS
EXPORT_ELEMENT(MiddleboxFW)
ELEMENT_MT_SAFE(MiddleboxFW)
