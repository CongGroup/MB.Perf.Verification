...
struct boxBatch
{
	...
	uint32_t batchSize;
	uint32_t batchPktSize;
	uint32_t packetCount;

	uint8_t rootPacket[1600];
	bool readyToSendRoot;
	MerkleTree tree;

	// flowid -- pktFlow
	std::unordered_map<uint32_t, pktFlow> flows;

	// pktID or flowid -- veriResult
	std::unordered_map<uint64_t, veriInfo> veriRes;
};
